...
import {
  ...
  questionTexts,
} from "../mock-data/mock-questionnaire-data";
import { hospitals, hospitalUnits } from "../mock-data/mock-hospital-data";
...
const QuestionnaireSummary: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    navigate(`/hospitalList?changesConfirmed=${hasEdited}`, {
      state: { userId },
    });
  };
  ...
  const handleEditClick = (editKeyId: string, event: React.MouseEvent) => {
    ...
    navigate(
      `/hospitalQuestionnaire/${hospitalUnitId}?editKeyId=${editKeyId}`,
      {
          ...
          <div className="nhsuk-grid-column-full">
            <div className="nhsuk-back-link mb-4">
              <button
                ...
                onClick={() =>
                  navigate(
                    `/hospitalQuestionnaire/${hospitalUnitId}`,
                    { state: { userId } }
                  )
                }
              >
